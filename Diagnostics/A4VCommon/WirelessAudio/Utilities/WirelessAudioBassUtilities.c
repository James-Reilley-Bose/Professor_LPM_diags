//
// WirelessAudioBassUtilities.c - bass utilities
//
...
static BOOL quickBoot = FALSE;
static uint8_t channel = WA_CHANNEL_INVALID;
static BOOL ColdbootInitCompleted = FALSE;

void WirelessAudioVariantUtils_Init(void)
{
    WirelessAudioVariantUtils_InitializeNV();
    SPEAKER_PACKAGE_VARIANT spv = NV_GetSpeakerPackageVariant();
    switch (spv) {
        case SPEAKER_PACKAGE_SKIPPER:
            channel = WA_CHANNEL_BASS_SKIPPER;
            break;
        case SPEAKER_PACKAGE_PBB:
            channel = WA_CHANNEL_BASS_PBB;
            break;
        default:
            channel = WA_CHANNEL_INVALID;
            break;
    }
}
...
BOOL WirelessAudioVariantUtils_InitializeDarr(void)
{
    ...
    quickBoot = (ConsoleDetect_IsConsoleCablePresent() ||
                (NV_GetRegionVariant() == REGION_JP));
    
    if(quickBoot)
    {
        LOG(wa_task, ROTTEN_LOGLEVEL_NORMAL, "Wired - DARR quick boot.");
    ...
    data = (region == REGION_JP) ? 0x80 : 0x10;
    ...
    data = (region == REGION_JP) ? 0x78 : 0xE1;
