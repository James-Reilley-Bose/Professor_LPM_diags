...
SCRIBE(sii9437_int, ROTTEN_LOGLEVEL_NORMAL);
...
extern I2C_BUS_HANDLE_TYPE SiiI2CBusHandle; // temp, move to task lb1302816
...
const uint8_t NUM_SII437_DEVS = 1; //lb1032816
		...
		const uint8_t *p_data, uint16_t size )
{
    ...
    debug_assert(dev_id < NUM_SII437_DEVS);

    // TODO: Pull this logic out into a function or macro
    uint8_t pRegAddr[2];
    pRegAddr[1] = (addr & 0xFF00) >> 8;
    pRegAddr[0] = (addr & 0x00FF);
    uint8_t i2c_addr = 0;
    switch(pRegAddr[1])
    {
        case 0x1:
            i2c_addr = SII9437_INST_1_I2C_ADDR_PAGE1;
            break;
        case 0x2:
            i2c_addr = SII9437_INST_1_I2C_ADDR_PAGE2;
            break;
        default:
            i2c_addr = SII9437_TOP_REGISTER_ADDR;
    }
    
    return i2cMaster_WriteRegister(SiiI2CBusHandle, i2c_addr, pRegAddr, 1, p_data, size);
}
		...
		uint8_t *p_data, uint16_t size )
{
    ...
    return i2cMaster_ReadRegister(SiiI2CBusHandle, i2c_addr, pRegAddr, 1, p_data, size);
}

void sii_platform_device_reset_set( uint32_t dev_id, bool_t b_on )
{
	//sii_platform_gpio_set(dev_id, SII_UC_RST,  !(b_on)); Go back and fix
}
