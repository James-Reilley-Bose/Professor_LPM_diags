...
void sii_sys_log_timestamp(const char *class_str, const char *func_str,
	void *p_obj)
{
	SII_SYS_TIME_MILLI_T milli_sec;
	uint16_t tot = 0;
	uint16_t ts_len = 0;
	char str[50];
	char t_len = 0;
	...
	milli_sec  = (SII_SYS_TIME_MILLI_T)sii_platform_time_msec_query();
	...
	/* Print time stamp */
	{
		...
		str[ts_len++] = '\n';
		str[ts_len++] = '[';
		t_len = SII_SPRINTF(&str[ts_len], sizeof(str) - ts_len, "%ld",
				milli_sec/1000);
		ts_len += t_len;
		str[ts_len++] = '.';
		t_len = SII_SPRINTF(&str[ts_len], sizeof(str) - ts_len, "%03ld",
				milli_sec%1000);
		ts_len += t_len;
		str[ts_len++] = ']';
		str[ts_len++] = ' ';
